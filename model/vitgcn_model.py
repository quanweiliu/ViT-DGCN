...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x)
        # print(x.shape)    # 8, 64, 6, 6
        x = x.flatten(2).transpose(1, 2)
        x = self.norm(x)
        return x

# if __name__ == "__main__":
#     input = torch.rand(8, 35, 27, 27)
#     model = PatchEmbed(img_size=27, patch_size=4, in_c=35, embed_dim=64)
#     output = model(input)
#     print(output.shape)   # 8, 36, 64
...
# if __name__ == "__main__":
#     input = torch.rand(8, 36, 64)
#     model = Attention(dim=64, num_heads=4)
#     output = model(input)
#     print(output.shape)   # 8, 36, 64
...
# if __name__ == "__main__":
#     input = torch.rand(8, 36, 64)
#     model = Mlp(in_features=64, hidden_features=64*4)
#     output = model(input)
#     print(output.shape)   # 8, 36, 64
...
class VisionTransformerGCN(nn.Module):
    def __init__(self, img_size=29, patch_size=4, in_c=5, num_classes=16,
        ...
        # GCN
        self.gcn = DynamicGraphConvolution(37, 64, 64)          # in, out, num_nodes
...
if __name__ == '__main__':
    img_size = 27
    model = VisionTransformerGCN(img_size=img_size, patch_size=4, in_c=32, 
