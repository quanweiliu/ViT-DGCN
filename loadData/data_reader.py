...
class PaviaCRaw(DataReader):
    def __init__(self, path_data=None):
        ...
        truth = sio.loadmat(os.path.join(path_data, "PaviaC_gt.mat"))
...
class Houston_2013Raw(DataReader):
    def __init__(self, path_data=None, type_data="Houston",):
        ...
        raw_data_package = sio.loadmat(os.path.join(path_data, "2013_DFTC/Houston_2013.mat"))
        self.data_cube = raw_data_package["Houston"].astype(np.float32)
        if type_data == "Houston":
            truth = sio.loadmat(os.path.join(path_data, "2013_DFTC/Houston_GT_2013.mat"))
            self.g_truth = truth["Houston_GT"]
        elif type_data == "TRLabel":
            truth = sio.loadmat(os.path.join(path_data, "2013_DFTC/TRLabel.mat"))
            self.g_truth = truth["TRLabel"]
        elif type_data == "TSLabel":
            truth = sio.loadmat(os.path.join(path_data, "2013_DFTC/TSLabel.mat"))
...
class Houston_2018Raw(DataReader):
    def __init__(self, path_data=None, type_data=None):
        ...
        raw_data_package = sio.loadmat(os.path.join(path_data, "2018IEEE_Contest/Houston2018/Houston.mat"))
        self.data_cube = raw_data_package["Houston"].astype(np.float32)
        if type_data == "Houston":
            truth = sio.loadmat(os.path.join(path_data, "2018IEEE_Contest/Phase2/hu2018_gt.mat"))
            self.g_truth = truth["hu2018_gt"]
        elif type_data == "TRLabel":
            truth = sio.loadmat(os.path.join(path_data, "2018IEEE_Contest/Houston2018/TRLabel.mat"))
            self.g_truth = truth["TRLabel"]
        elif type_data == "TSLabel":
            truth = sio.loadmat(os.path.join(path_data, "2018IEEE_Contest/Houston2018/TSLabel.mat"))
            self.g_truth = truth["TSLabel"]
        else:
            raise KeyError("Please select among ['Houston', 'TRLabel', 'TSLabel']")
...
#     def __init__(self, path_data=None,num_classes = 8):
#         super(CuonadongRaw, self).__init__()

#         path = '/home/yangzhenzhen/data/ZZ_VIGCN/data/out.mat' 
#         raw_data_package = hdf5storage.loadmat(path)
#         # raw_data_package = sio.loadmat(path_data + "sb.mat")
#         self.data_cube = raw_data_package["data"].astype(np.float32)
#         truth = sio.loadmat(path_data + "out_gt.mat")
#         self.g_truth = truth["groundT"]
#         # if num_classes == 2:
#         #     self.g_truth[ self.g_truth !=7 ] =0
#         #     self.g_truth[ self.g_truth == 7] =1
            


# Load the dataset
def load_data(dataset = "IndianPines", path_data=None, type_data="Houston",num_classes=2):
    ...
    # if dataset == "Cuonadong":
    #     dataset = CuonadongRaw(path_data,num_classes)
    #     data =dataset.normal_cube
    #     data_gt = dataset.truth
